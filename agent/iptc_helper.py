...
from iptc.ip4tc import Rule, Table, Chain
from iptc.ip6tc import Rule6, Table6
...
def decode_iptc_rule(iptc_rule, ipv6=False):
    ...
    d = {}
    if ipv6 is False and iptc_rule.src != '0.0.0.0/0.0.0.0':
        _ip, _netmask = iptc_rule.src.split('/')
        _netmask = _netmask_v4_to_cidr(_netmask)
        d['src'] = '{}/{}'.format(_ip, _netmask)
    elif ipv6 is True and iptc_rule.src != '::/0':
        d['src'] = iptc_rule.src
    if ipv6 is False and iptc_rule.dst != '0.0.0.0/0.0.0.0':
        _ip, _netmask = iptc_rule.dst.split('/')
        _netmask = _netmask_v4_to_cidr(_netmask)
        d['dst'] = '{}/{}'.format(_ip, _netmask)
    elif ipv6 is True and iptc_rule.dst != '::/0':
        d['dst'] = iptc_rule.dst
    ...
    if ipv6 is False and iptc_rule.fragment:
...
def _iptc_table_available(table, ipv6=False):
    ...
    try:
        Table6(table) if ipv6 else Table(table)
        return True
    except Exception:
        return False
